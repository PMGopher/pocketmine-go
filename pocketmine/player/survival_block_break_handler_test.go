...
func TestNewSurvivalBlockBreakHandlerComputesAPositiveBreakSpeedForABreakableBlock(t *testing.T) {
	...
	h := NewSurvivalBlockBreakHandler(p, math.NewVector3(0, 70, 1), block.VanillaStone(), math.Up, 7, DefaultFxIntervalTicks)
...
func TestNewSurvivalBlockBreakHandlerIsZeroForAnUnbreakableBlock(t *testing.T) {
	p := newTestPlayer(t, 1, math.NewVector3(0, 70, 0))
	h := NewSurvivalBlockBreakHandler(p, math.NewVector3(0, 70, 1), block.VanillaBedrock(), math.Up, 7, DefaultFxIntervalTicks)
	if got := h.GetBreakSpeed(); got != 0 {
...
func TestUpdateAccumulatesBreakProgressUntilComplete(t *testing.T) {
	...
	h := NewSurvivalBlockBreakHandler(p, math.NewVector3(0, 70, 0), block.VanillaStone(), math.Up, 7, DefaultFxIntervalTicks)
...
func TestUpdateReturnsFalseWhenPlayerIsTooFarAway(t *testing.T) {
	p := newTestPlayer(t, 1, math.NewVector3(0, 70, 0))
	h := NewSurvivalBlockBreakHandler(p, math.NewVector3(100, 70, 100), block.VanillaStone(), math.Up, 7, DefaultFxIntervalTicks)
